...
public final class SimpleObjectSut {
    ...
    public void check() {
        if (value < 0) {
            throw new IllegalStateException("How could it become 0? Dump: " + toString());
        }

        for (Long divisor : divisors) {
            if (divisor == null) {
                throw new IllegalStateException("How could it become null? Dump: " + toString());
            } else if (divisor < 0) {
                throw new IllegalStateException("How could it become negative? Dump: " + toString());
            } else if (value % divisor != 0) {
                throw new IllegalStateException("How could it become invalid? Dump: " + toString());
            }
        }
    }
